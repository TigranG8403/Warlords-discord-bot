...
RULES_PANELS_LEGACY_PATH = PROJECT_ROOT / "data" / "rules_panels.json"


def _build_rules_embed(*, image_url: str | None) -> discord.Embed:
...
def build_module() -> BotModule:
    repository = PublishedPanelRepository(
        ...
        namespace="rules",
        legacy_path=RULES_PANELS_LEGACY_PATH,
    )
    ...
    def render_panel(_record: PublishedPanelRecord, _channel: discord.TextChannel) -> PanelRenderResult:
        ...
        return PanelRenderResult(
            embed=_build_rules_embed(image_url=image_url),
    ...
    def register(bot: commands.Bot) -> None:
        ...
        rules_group = app_commands.Group(name="rules", description=GROUP_DESCRIPTION)

        @rules_group.command(name="panel", description=COMMAND_DESCRIPTION)
